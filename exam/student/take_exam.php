    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            foreach ($_POST['answers'] as $question_id => $answer_id) {
                file_put_contents('debug_loop.log', "QID: $question_id, AID: $answer_id\n", FILE_APPEND);
                if (!empty($answer_id)) {
            ...
            file_put_contents('debug_db.log', $e->getMessage(), FILE_APPEND);
            $pdo->rollBack();
...
<body class="exam-page">
    ...
    <script>
        ...
        function nextQuestion() {
            ...
            const questionId = currentQuestionCard.querySelector('[data-question-id]')?.getAttribute('data-question-id');
            
            if (questionId) {
                const selectedAnswer = document.getElementById('input_' + questionId).value;
