...
if (!$attempt) {
    ...
    // Get questions and student answers (answer_id)
    $stmt = $pdo->prepare("
        SELECT q.*, sa.answer_text as student_answer_id
        FROM questions q
    ...
    // Get correct answers as answer_id
    $stmt = $pdo->prepare("
    ...
    // For each question, fetch all possible answers
    foreach ($questions as &$question) {
        ...
        $question['correct_answer_id'] = $correct_answers[$question['question_id']] ?? null;
        $question['is_correct'] = (
            $question['student_answer_id'] !== null &&
            intval($question['student_answer_id']) === intval($question['correct_answer_id'])
        );
    }
    unset($question);
    ...
    $correct_count = 0;
    foreach ($questions as $question) {
        if ($question['is_correct']) $correct_count++;
    }
                        ...
                        <div class="question-content">
                            ...
                            <div class="answer-review">
                                ...
                                foreach ($question['answers'] as $answer):
                                    if (in_array($answer['answer_id'], $shown)) continue;
                                    $shown[] = $answer['answer_id'];
                                    $is_student_answer = $answer['answer_id'] == $question['student_answer_id'];
                                    $is_correct_answer = $answer['answer_id'] == $question['correct_answer_id'];
                                    $class = '';
                                ...
                                ?>
                                    <div class="answer-option <?php echo $class; ?>">
