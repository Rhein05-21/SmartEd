...
<body class="min-h-screen flex items-center justify-center">
    <div class="container-auth" id="container">
        <div class="forms-container">
            <form class="form" method="POST" autocomplete="off">
                ...
                <!-- OTP segmented input -->
                <div id="otp" class="flex form-otp text-center pb-3 w-full justify-between mb-3">
                    <input class="text-center form-control border border-gray-300 rounded bg-gray-50 focus:border-purple-400 text-base font-mono w-8 h-8 mx-0.5" type="text" id="digit-1" name="digit-1" data-next="digit-2" maxlength="1" required />
                    <input class="text-center form-control border border-gray-300 rounded bg-gray-50 focus:border-purple-400 text-base font-mono w-8 h-8 mx-0.5" type="text" id="digit-2" name="digit-2" data-next="digit-3" data-previous="digit-1" maxlength="1" required />
                    <input class="text-center form-control border border-gray-300 rounded bg-gray-50 focus:border-purple-400 text-base font-mono w-8 h-8 mx-0.5" type="text" id="digit-3" name="digit-3" data-next="digit-4" data-previous="digit-2" maxlength="1" required />
                    <input class="text-center form-control border border-gray-300 rounded bg-gray-50 focus:border-purple-400 text-base font-mono w-8 h-8 mx-0.5" type="text" id="digit-4" name="digit-4" data-next="digit-5" data-previous="digit-3" maxlength="1" required />
                    <input class="text-center form-control border border-gray-300 rounded bg-gray-50 focus:border-purple-400 text-base font-mono w-8 h-8 mx-0.5" type="text" id="digit-5" name="digit-5" data-next="digit-6" data-previous="digit-4" maxlength="1" required />
                    <input class="text-center form-control border border-gray-300 rounded bg-gray-50 focus:border-purple-400 text-base font-mono w-8 h-8 mx-0.5" type="text" id="digit-6" name="digit-6" data-previous="digit-5" maxlength="1" required />
                </div>
                <input type="hidden" name="reset_otp" id="reset_otp" />
    ...
    inputs.forEach((input, idx) => {
        input.addEventListener('input', function(e) {
            ...
            if (this.value.length === 0 && this.dataset.previous && e.inputType === 'deleteContentBackward') {
    ...
    // On form submit, concatenate digits into hidden reset_otp
    const form = document.querySelector('form');
    form.addEventListener('submit', function(e) {
        let otp = '';
        inputs.forEach(input => otp += input.value);
        document.getElementById('reset_otp').value = otp;
    });
    </script>
</body>
</html> 